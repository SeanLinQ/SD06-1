...
uint8_t angleBackup = 40;     //保存设置最低亮度时的当前亮度值
uint32_t angleMinBackup = 40; //保存设置最低亮度时的当前最低亮度值
...
uint16_t pwm_delay=40;
uint16_t backup_delay = 0;
uint16_t pwm_pulse_width = 40; 
uint32_t light_angle = 40;
uint32_t backup_light_angle = 40;
uint8_t peroid_count = 0;
uint8_t period_flag = FALSE;

uint32_t brightness_adjust = 40;
...
uint8_t zcd_flag = FALSE;
uint8_t zcd_flag1 = FALSE;
...
//		//调光数据
		...
		{
			  SN_CT16B2->MR9 = frq_period;
			  SN_CT16B2->MR2 = frq_period;
...
//		}
		...
		//pwm_pulse_width = 3000;
		
		SN_CT16B2->PWMCTRL_b.PWM2MODE = 1;
		SN_CT16B2->MR2 = pwm_delay+250;
		SN_CT16B2->MR9 = pwm_delay+2450;		
		SN_CT16B2->TC = 0;
...
//		zcd_flag1 = TRUE;
//		UT_DelayNx10us(pwm_delay);
//		DIM_H;
//		UT_DelayNx10us(100);
//		DIM_L;
		//backup_delay = pwm_pulse_width+pwm_delay;
		//CT16B1_NvicEnable();
...
static void Key_Power_ShortPress_Handle(void)
{
	if(Device_Param.led_switch == OFF) 
	{
		...
		//if(DevMode == DEV_MODE_NORMAL)
		{
			...
			mcu_dp_bool_update(DPID_LED_SWITCH_1, Device_Param.led_switch);		
		}
	}
	else
	{
...
void Led_Process(void)
{
	if(Device_Param.led_switch == ON )
	{
		...
		if(Device_Param.bright_value >= 10 && Device_Param.bright_value <= 200)
		{
			LED1_ON;
			LED2_OFF;
			LED3_OFF;
			LED4_OFF;
			LED5_OFF;
		}else if(Device_Param.bright_value > 200 && Device_Param.bright_value <= 400)
    {
			LED1_ON;
			LED2_ON;
			LED3_OFF;
			LED4_OFF;
			LED5_OFF;
		}else if(Device_Param.bright_value > 400 && Device_Param.bright_value <= 600)
		{
			LED1_ON;
			LED2_ON;
			LED3_ON;
			LED4_OFF;
			LED5_OFF;
		}else if(Device_Param.bright_value > 600 && Device_Param.bright_value <= 800)
		{
			LED1_ON;
			LED2_ON;
			LED3_ON;
		...
		else if(Device_Param.bright_value > 800 && Device_Param.bright_value <= 1000)
		{
			LED1_ON;
			LED2_ON;
			LED3_ON;
